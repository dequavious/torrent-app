...
class File:
    def track_progress(self):
        if not (self.root.stopped and self.root.torrent.seeding_or_stopped()):
            try:
                progress = self.root.torrent.get_file_progress(self.index)
                percentage = int(round((progress / self.file.size) * 100))
            ...
            self.frame.after(1000, self.track_progress)  # Repeat after 1000 milliseconds (1 second)
    ...
    def __init__(self, root, frame, file, index):
        ...
        self.file = file
        self.index = index
        self.last_percentage = 0  # Keep track of the last progress percentage
        ...
        self.frame.after(0, self.track_progress)
...
class TorrentSettings:
    def add_trackers(self):
        ...
        if trackers != '':
            ...
            list(map(lambda tracker: self.tree.insert(parent='', index='end', values=(tracker,)),
                     trackers.split("\n")))
    ...
    def refresh_peer_list(self):
        ...
        list(map(lambda peer: self.tree2.insert(parent='', index='end', values=(f"{peer.ip[0]}",)), peers))
    ...
    def check_stopped(self):
        try:
            if self.parent.stopped:
                ...
                self.window.after(1000, self.check_stopped)
    ...
    def __init__(self, parent):
        ...
        list(map(lambda tracker: self.tree.insert(parent='', index='end', values=(f"{tracker}",)), trackers))
        ...
        self.window.after(0, self.check_stopped)
...
class Download:
    ...
    def track_progress(self):
        if not (self.stopped and self.torrent.seeding_or_stopped()):
            status = self.torrent.handle.status()
            if not self.stopped:
                self.update_progress_bar(status)
            self.frame.after(1000, self.track_progress)
    ...
    def pause_resume(self, action):
        if action == 1:
            ...
            self.frame.after(0, self.torrent.resume)
        else:
            self.paused = True
            ...
            self.frame.after(0, self.torrent.pause)
    ...
    def __init__(self, parent, torrent, frame, number, style):
        ...
        self.settings_btn = Button(self.frame, image=self.SETTINGS_IMG, compound=CENTER,
                                   command=lambda: TorrentSettings(self),
                                   state='disabled')
...
class GUI:
    ...
    def scrape(self, search, page):
        if search != '':
            ...
            list(map(lambda args: self.tree.insert(parent='', index='end', iid=f'{args[0]}', values=(args[1]['name'], args[1]['seeds'], args[1]['leech'], args[1]['size'])), enumerate(self.results)))
    ...
    def start_download(self, torrent, i=None):
        if i is not None:
            Thread(target=Download, daemon=True, args=(self, torrent, self.download_frame.scrollable_frame, i,
                                                            self.style)).start()
        else:
            Thread(target=Download, daemon=True,
    ...
    def __init__(self):
        ...
        self.add_menu.add_command(label='Magnet', command=lambda: Magnet(self))
        self.add_menu.add_command(label='Torrent', command=lambda: Thread(target=self.open_torrent,
                                                                          daemon=True).start())
        self.menu.add_command(label='Settings', command=lambda: GlobalSettings(self))
